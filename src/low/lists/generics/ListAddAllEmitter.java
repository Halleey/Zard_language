...
public class ListAddAllEmitter {
    ...
    public LLVMValue emit(ListAddAllNode node, LLVisitorMain visitor) {
        ...
        if (n == 0) {
            return node.getTargetListNode().accept(visitor);
        }

        // Descobrir tipo SEM poluir código
        ...
        if (firstType.equals("%String*")) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                llvm.append("  store %String* ").append(val.getName())
        ...
        if (firstType.equals("i8*")) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                llvm.append("  store i8* ").append(val.getName())
