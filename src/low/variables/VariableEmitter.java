...
public class VariableEmitter {
    ...
    private final StringCopyEmitter stringCopyEmitter;

    public VariableEmitter(
        ...
        this.expressionInitEmitter =
                new ExpressionInitEmitter(temps, visitor, storeEmitter, tempExtractor);
        this.stringCopyEmitter = new StringCopyEmitter(temps);
    }
    ...
    public String emitInit(VariableDeclarationNode node) {
        ...
        if (srcType != null && srcType.startsWith("Struct<")) {
            ...
            if (node.initializer instanceof StructInstaceNode sin) {
                ...
                if (sin.getNamedValues().isEmpty()
                        && sin.getPositionalValues().isEmpty()) {
                    return structInitEmitter.emit(node, info);
                }
        ...
        if ("string".equals(info.getSourceType())) {
            ...
            String exprCode = node.initializer.accept(visitor);
            String srcTemp = extractTemp(exprCode);

            String copyCode = stringCopyEmitter.emit(srcTemp);
            String newStr = extractTemp(copyCode);
    ...
    private String handleDefaultInit(
    ...
    private String handleInputInit(
        ...
        InputEmitter inputEmitter =
                new InputEmitter(temps, visitor.getGlobalStrings());
        ...
        if ("string".equals(info.getSourceType())) {
            ...
            String copyCode = stringCopyEmitter.emit(temp);
            String newStr = extractTemp(copyCode);

            return code
                    + copyCode
                    + "  store %String* " + newStr
                    + ", %String** " + getVarPtr(node.getName()) + "\n";
        }

        return code
                + "  store " + info.getLLVMType() + " " + temp
                + ", " + info.getLLVMType() + "* "
                + getVarPtr(node.getName()) + "\n";
    }
    ...
    private String handleListLiteralInit(
        ...
        visitor.registerListElementType(
                node.getName(),
                info.getElementType()
        );
        ...
        return switch (info.getSourceType()) {
            ...
            case "List<boolean>" -> {
                ...
                String code = e.emit(listNode, visitor);
                yield code
                        + "  store %struct.ArrayListBool* "
                        + extractTemp(code)
                        + ", %struct.ArrayListBool** "
                        + varPtr + "\n";
            }
            ...
            case "List<double>" -> {
                ...
                String code = e.emit(listNode, visitor);
                yield code
                        + "  store %struct.ArrayListDouble* "
                        + extractTemp(code)
                        + ", %struct.ArrayListDouble** "
                        + varPtr + "\n";
            }
            ...
            default -> {
                ...
                yield code
                        + "  " + casted
                        + " = bitcast i8* " + tmp + " to %ArrayList*\n"
                        + ";;VAL:" + casted + ";;TYPE:%ArrayList*\n"
                        + "  store %ArrayList* " + casted
                        + ", %ArrayList** " + varPtr + "\n";
            }
        };
    }
    ...
    private String callArrayListCreateIntAndStore(String varPtr) {
        String tmp = temps.newTemp();
        return "  " + tmp
                + " = call %struct.ArrayListInt* @arraylist_create_int(i64 4)\n"
                + ";;VAL:" + tmp + ";;TYPE:%struct.ArrayListInt*\n"
                + "  store %struct.ArrayListInt* " + tmp
    ...
    private String callArrayListCreateDoubleAndStore(String varPtr) {
        String tmp = temps.newTemp();
        return "  " + tmp
                + " = call %struct.ArrayListDouble* @arraylist_create_double(i64 4)\n"
                + ";;VAL:" + tmp + ";;TYPE:%struct.ArrayListDouble*\n"
                + "  store %struct.ArrayListDouble* " + tmp
    ...
    private String callArrayListCreateBoolAndStore(String varPtr) {
        String tmp = temps.newTemp();
        return "  " + tmp
                + " = call %struct.ArrayListBool* @arraylist_create_bool(i64 4)\n"
                + ";;VAL:" + tmp + ";;TYPE:%struct.ArrayListBool*\n"
                + "  store %struct.ArrayListBool* " + tmp
    ...
    public String getVarPtr(String name) {
        ...
        if (ptr == null) {
            throw new RuntimeException(
                    "Ptr não encontrado para variável: " + name
            );
        }
        return ptr;
    }
